...
let ProductsComponent = class ProductsComponent {
    constructor(modalService, imgService, shop) {
    ...
    addToCart() {
        const modalRef = this.modalService.open(_shoppingcart_shoppingcart_component__WEBPACK_IMPORTED_MODULE_5__["comfirmQuantity"]);
...
let PedidosCComponent = class PedidosCComponent {
    constructor() {
...
let ShoppingService = class ShoppingService {
    constructor(http, auth, router, toastr) {
        this.http = http;
        this.auth = auth;
        this.router = router;
        this.toastr = toastr;
    ...
    addToCart(pId, spId, pName, pPrice, pQuantity, img) {
        if (this.auth.isTokenValid()) {
    ...
    makeOrder(order) {
        this.http.post(`${this.BaseURL}/ORDERS/create`, order, this.httpOptions).subscribe((data) => {
...
let comfirmQuantity = class comfirmQuantity {
    constructor(activeModal, shop) {
...
comfirmQuantity.ctorParameters = () => [
...
let ShoppingcartComponent = class ShoppingcartComponent {
    constructor(router, modalService, shop) {
        this.router = router;
        this.modalService = modalService;
        this.shop = shop;
    }
...
let shopElement = class shopElement {
};
tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"]([
    Object(_angular_core__WEBPACK_IMPORTED_MODULE_1__["Input"])()
...
let UsersService = class UsersService {
    constructor(http, router, auth, toastr) {
        this.http = http;
        this.router = router;
        this.auth = auth;
        this.toastr = toastr;
    ...
    registerUser(user) {
        //console.log('LLEGÓ');
        this.http.post(`${this.BaseURL}/USERS/register`, user, this.httpOptions).subscribe((data) => {
            console.log(data);
            if (data["result"].includes("Successful")) {
                this.router.navigate(['/login']);
            }
